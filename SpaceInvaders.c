...
int Screen_divisions[3]={20,40,60}; // the coordinates of X-axis that the Car is moving on 
...
int main(void){
  ...
  Nokia5110_Init();
	////////////////////////////////////////////////////
	flagSW1=0,flagSW2=0,caX=40,caY=47;
	////////////////////////////////////////////////////////
	points=0;
	//int Screen_divisions[3]={20,40,60};
	// ******************** initializations ***************************** //
	PortF_Init();
	caY=47;
	caX=40;
	enefrontX = 48- enemyHeight;
	eneX2 = 48- enemyHeight;
	eneNEWY = rand()%3;
...
// SW2  is gonna move the car downward
	...
	while(true){
		if(!((GPIO_PORTF_DATA_R&0x1)))break; // Check if SW2 is pressed , it sends 0 
//			Since  !0=1  and *****if(1)******  break to the game
			
  }
	Nokia5110_ClearBuffer();
	Nokia5110_PrintBMP(40,47, car, 0);
	...
	while(true){
		...
		if(!(((GPIO_PORTF_DATA_R&(1<<4))==0) && ((GPIO_PORTF_DATA_R&0x1)==0  ))){
			...
			if(((GPIO_PORTF_DATA_R&0x1) == 0)){ // check SW2 is pressed ?
				if((caX < 60) && (flagSW2==0)){ // if the car is found between 0 to 28 coordinates of Y axis
		...
		Nokia5110_PrintBMP(caX,47 , car, 0);
			//Delay100ms(20);
		Nokia5110_DisplayBuffer();
		

		if(enefrontX != 48){
			Nokia5110_PrintBMP( Screen_divisions[eneNEWY],enefrontX, SmallEnemy20PointA, 0);
			enefrontX++;
		}
		...
		if(enefrontX == 48){
			...
			Nokia5110_PrintBMP(Screen_divisions[eneNEWY],eneX2 , SmallEnemy20PointA, 0);
		}
		if((eneX2 == caY) && (Screen_divisions[eneNEWY] == caX)){
			break;
		}
		eneX2++;
